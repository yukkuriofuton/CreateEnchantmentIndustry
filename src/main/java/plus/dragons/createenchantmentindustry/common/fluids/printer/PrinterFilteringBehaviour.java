...
import plus.dragons.createenchantmentindustry.common.fluids.printer.behaviour.PrintingRecipeBehaviour;
...
public class PrinterFilteringBehaviour extends FilteringBehaviour {
    ...
    public PrinterFilteringBehaviour(SmartBlockEntity be, SmartFluidTankBehaviour tank, ValueBoxTransform slot) {
        super(be, slot);
        this.tank = tank;
    }
    ...
    public boolean setFilter(ItemStack stack) {
        if (super.setFilter(stack)) {
            printing = PrintingBehaviour.create(getWorld(), tank, stack);
            return true;
        }
        return false;
    }
    ...
    public void read(CompoundTag nbt, Provider registries, boolean clientPacket) {
        ...
        printing = PrintingBehaviour.create(getWorld(), tank, filter.item());
    }
